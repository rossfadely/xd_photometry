...
# last too functions added to allow fixed means and axis-aligned
...
def set_new_GMM(old_model):
    """
    Initialize a new model with the previous parms.
    """
    model = GMM(n_components=old_model.n_components, covariance_type='full',
                n_iter=1, n_init=0)
    model.means_ = old_model.means_
    model.covars_ = old_model.covars_
    model.weights_ = old_model.weights_
    return model
